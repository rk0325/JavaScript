...
また、抽出したプロパティに別名をつけたい場合は以下のように**:(コロン)**を使用することでその変数名で扱うことも可能
...
オブジェクト内の値を使っていく場合は、分割代入を用いてよりシンプルに記述できないか考えてみるといい
...
変数宣言部に**[ ]**を使用して、配列に格納されている順番に任意の変数名を設定して抽出することができる
...
変数名の後ろに **=** で値を設定しておくと、プロパティが存在しない場合に設定する値を指定できる
...
スプレッド構文を用いて配列やオブジェクトのコピー、結合もできる

**…**で順番に展開して、**[ ]**で囲んでいるので結果的に新しい配列ができる

⚠️**＝でコピーしてはいけない**

配列やオブジェクトなどの「オブジェクト型」と呼ばれる変数はイコールでコピーすると**参照値も引き継がれてしまう**ため、予期せぬ挙動が起きてしまうことがある

**JavaScriptにおける配列（Array）とオブジェクト（Object）の基本的な違いについて**

### **配列 (Array)**

1. **順序付けられたコレクション**: 配列は順序付けられたデータのコレクションです。各データは特定のインデックスでアクセスされます。
2. **インデックス**: 通常、0から始まる数字を使用してデータにアクセスします。
...
3. **組み込みメソッド**: `push()`, `pop()`, `shift()`, `unshift()`, `splice()` などの組み込みメソッドが利用できます。
4. **長さのプロパティ**: `length` プロパティを使用して配列の長さを取得できます。
...
1. **キーと値のペア**: オブジェクトはキーと値のペアで構成されています。
2. **キーのアクセス**: 文字列やシンボルをキーとして使用してデータにアクセスします。
...
3. **メソッドとプロパティ**: オブジェクトはプロパティとメソッドを持つことができます。
...
4. **動的性**: キーと値のペアを動的に追加・削除できます。
    
    ```jsx
    person.country = "Japan"; // 新しいキーと値のペアを追加
    delete person.age; // キーと値のペアを削除
    ```
    

### **主な違い:**

1. 配列は順序付けられたデータのコレクションであり、インデックスによってアクセスされます。一方、オブジェクトはキーと値のペアのコレクションであり、キーによってアクセスされます。
2. 配列には組み込みの多くのヘルパーメソッドがありますが、オブジェクトにはそのような多くのヘルパーメソッドはありません。

これらの基本的な違いを理解することで、どちらを使用すべきか、または特定のタスクに最適なのかを判断することができるようになります。

### map関数

配列に対して、配列.map( )という形で使用していく
...
{ name: '津崎', age: 27 }
{ name: 'Nushida', age: 27 }
{ name: 'Nushida', age: 27, address: 'Tokyo' }
```
